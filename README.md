...
- **键盘优先**：终端打开/切换标签/挂起恢复时**自动聚焦**，把键盘交给终端（以终端输入为准，避免 `Ctrl+C`/`Ctrl+U`/`Ctrl+W`/`Ctrl+A` 等被浏览器抢走）。标签栏**双击空白区**切换**接管/释放**；终端**聚焦时**标签栏右侧浮现一个**极简图钉 `📌`**，点它即释放/接管。注：`Ctrl+T/Ctrl+W/Ctrl+L`、`Cmd+W`、`F5/Ctrl+R`、`Ctrl+Shift+I` 等**窗口级**快捷键由浏览器 chrome 处理，页面无法拦截（释放后这些才会回给浏览器）。
