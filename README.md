...
- **入口**：对话区底部有一根 **iOS App Switcher 样式的透明横杠**：**上滑**（或轻点/回车）**打开**终端。它在对话区底部预留了一条 `HANDLE_STRIP_H`（14px）横条，**不遮挡 dsh 输出统计**；**3 秒无操作自动淡出**、光标靠近底部重新亮起（同滚动条逻辑）。**收起终端用面板顶部的小横杠**（拖动=调高、点击=收起），而不是这根入口横杠。头部不放终端按钮，保持简洁。
