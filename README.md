...
- **拖拽调高**：终端顶部拖拽条可调高度（120–520px）。**视觉上是一根 1px 细线**（与面板边框同色，安静不抢视线），但**命中区有 9px 高**（视觉与手感解耦，学官方做法）；**鼠标靠近或拖动时**细线亮成品牌色、中间浮出一根圆角把手提示"这里能抓"；**触摸端**（无 hover）把手常驻淡显。**点击（未拖动）仍然是收起终端**。
